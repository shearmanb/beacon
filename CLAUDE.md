...
- **Wedge-proof worker + fallback newest-first (2026-07-22)**: post-mortem of the Jul 21–22 silent blackout (missed the Reveries 10yr Glaze/ENDALZ SharedPour drop; the daily mirror commits stopped after Jul 20 22:21 ET while the dashboard stayed up). Root cause class: the loop had **no recovery path for a pass that never finishes** — `serve.ts` fire-and-forgot `startLoop` (`void`, no `.catch`), the 2e guards swallow `uncaughtException` (so a broken promise chain leaves the loop hung forever inside a live process), the mirror's GitHub calls were the loop's only unbounded HTTP (bare `fetch`, no timeout), and Railway's ON_FAILURE restart never fires because the process never exits. Fixes: (1) **wedge watchdog** in `loop.ts` — if no pass completes in 15 min (`WEDGE_LIMIT_MS`, well past the worst legitimate all-sites-failing pass), record a `self_healed` history row (dashboard-only, per the 2026-07-19 policy) and `process.exit(1)` so Railway restarts the service and checks resume unattended; (2) `serve.ts` `.catch`es a loop rejection → exit(1) — a dashboard quietly serving with a dead worker is the worst failure mode a monitor has; (3) mirror GET/PUT hard-capped at 20s via `AbortSignal.timeout`; (4) **Storefront fallback sorts newest-first** (root `CREATED_AT` / collection `CREATED`, `reverse: true`) — the API's default id-ascending sort meant a truncated roster hid exactly the *newest* products, i.e. drops; now truncation can only shed the oldest tail and a new listing is always on page 1. Ops follow-ups (no code): set `HEALTHCHECK_URL` (the external dead-man would have caught this in minutes — the dashboard still nags while unset), and if mirror commits stay absent after this deploy, check the `GH_TOKEN` PAT expiry (R4).
