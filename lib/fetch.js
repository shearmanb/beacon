...
export function https(url, options = {}, _visited = new Set(), _retries = 0) {
  return new Promise((resolve, reject) => {
    ...
    const { ua, ...chHeaders } = randomFrom(BROWSER_PROFILES);
    ...
    const req = request(reqOptions, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        _visited.add(url);
        res.resume();
        resolve(https(next, options, _visited, _retries));
        return;
      }
      ...
      if ((res.statusCode === 429 || res.statusCode === 503) && _retries < 3) {
        ...
        setTimeout(() => resolve(https(url, options, _visited, _retries + 1)), delay);
        return;
      }
